...
| vm_mark p r s a c c' q : ⟨MARK r c' c, a, p, q, s⟩ ==> ⟨c, a, Some r, q, s[r:= HAN c' p]⟩
...
(** - [x = Catch x1 x2]: *)
  ...
  <== {eapply vm_unmark}
      match eval e1 q with
      | (Some n, q') => ⟨UNMARK c , n,Some r, q', s[r:= HAN (comp' e2  r c) p]⟩
      | (None, q') => ⟪Some r, q', s[r:= HAN (comp' e2  r c) p]⟫
      end.
  <|= {apply IHe1}
      ⟨comp' e1 (next r) (UNMARK c) , a ,Some r, q, s[r:= HAN (comp' e2  r c) p]⟩.
  <== {apply vm_mark}
