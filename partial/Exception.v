...
(using PANIC instruction to signal uncaught exceptions). *)
...
(** - [x = Catch x1 x2]: *)
  ...
  <== {eapply vm_unmark}
      match eval e1 with
      | Some n => ⟨UNMARK c, n, (comp e2 r c,r), s[r:= HAN h]⟩
      | None => ⟪(comp e2 r c,r), s[r:= HAN h]⟫
      end.
  <|= {apply IHe1}
      ⟨comp e1 (next r) (UNMARK c) , a ,(comp e2 r c,r), s[r:= HAN h]⟩.
  <== {apply vm_mark}
...
Theorem sound' x a r C :
  ⟨compile x, a, (PANIC, r), empty⟩ =>>! C ->
  ((exists m n, C = ⟨HALT, n, (PANIC, r), m⟩ /\ eval x = Some n)
   \/ ((forall n h m, C <> ⟨HALT, n, h, m⟩) /\ eval x = None)).
Proof.
  intros E.
  pose (spec' x a (PANIC, r)). unfold Reach in *. repeat autodestruct.
  ...
  - right. inversion H0. subst. inversion H1. subst.
    remember (get r s') as G. split;auto.
    destruct G. destruct r0.
    + assert (C = ⟪ (PANIC, r), s' ⟫) as R. eapply D. eapply E. split. auto. intro Contra. destruct Contra.
      inversion H3. rewrite H9 in HeqG. inversion HeqG. subst. intros. intro Contra. inversion Contra.
    + assert (C = ⟨ PANIC, 0, h, s' ⟩) as R.
