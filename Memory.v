...
Definition Mem (T : Type) : Type := adr -> T.

Definition get {T} (r : adr) (m : Mem T) : T := m r.

Definition set {T} (r : adr) (v : T) (m : Mem T) : Mem T :=
...
Lemma eqr_less T (x y : Mem T) r r' : x =[ r ] y -> r' <= r -> x =[ r' ] y.
Proof. intros R L i L'. apply R. omega. Qed.


Lemma set_get_neq T r i (s : Mem T) n : r <> i -> get i s = get i (set r n s).
Proof.
...
Lemma set_get T r (s : Mem T) n : get r (set r n s) = n.
Proof.
...
Lemma set_eqr_incr T r (s s' : Mem T) n : s =[r] s' -> set r n s =[r + 1] set r n s'.
Proof.
  admit.
Admitted.
